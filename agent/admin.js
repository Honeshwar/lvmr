function getValueAtPath(obj, path) {
  return path.reduce((current, key) => (current && key in current ? current[key] : undefined), obj);
}
...
function isRateObject(obj) {
  return obj && typeof obj === 'object' && !Array.isArray(obj) && Object.values(obj).every(value => typeof value === 'number');
}
...
function buildRoomCard(parent, roomKey, roomData, parentPath) {
  ...
  if (isRateObject(roomData)) {
    ...
    Object.entries(roomData).forEach(([planKey, planData]) => {
      buildPlanRow(roomCard, planData, [...parentPath, roomKey, planKey], planKey);
    });
  }
...
async function loadConfig() {
  ...
  try {
    ...
    updateRawJson(data);
    showStatus('Loaded configuration successfully. Edit values and download JSON to update the repository.', true);
  } catch (error) {
    console.error(error);
    editor.innerHTML = '<p style="color:#b91c1c;">Failed to load config. Make sure the file exists at <code>agent/calculator-prices.json</code> and the page is served over HTTP.</p>';
  ...
  downloadBtn.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('/.netlify/functions/save-prices', {
        ...
        body: JSON.stringify({ prices: data, commitMessage: 'Update calculator prices from admin UI' }),
      });
      ...
      showModal('Saved to GitHub', result.message || 'Your pricing file was committed.', result.commitUrl);
    } catch (error) {
      console.error(error);
      showStatus(error.message, false);
